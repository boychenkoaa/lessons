"""
Решение не топорное, но и без полноценного анализатора.

Сам робот отвечает только за свое состояние и поддерживает базовые методы.
Вся остальная "обвязка" ложится на команды (наследники RobotCmd): перевод градусы / радианы, печать результатов команды.
Команды также парсят сами себя из строки -- это тоже их ответственность.

Интерпретирует построчно. Одна команда -- одна строка.
Циклы и условные переходы мое решение уже не потянет, только линейные программы.
Хотя -- можно же сделать вложенные команды...
"""
