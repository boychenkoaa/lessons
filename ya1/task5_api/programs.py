...
def program_goto_pos(pos: Pos, target: Pos) -> list[str]:
    dx = target.x - pos.x
    dy = target.y - pos.y
    dist = hypot(dx, dy)
    if dist == 0:
        return [f"turn {target.angle - pos.angle}"]

    move_angle = degrees(atan2(dy, dx))
    turn_before = move_angle - pos.angle
...
def program_rect_abs(pos: Pos, rect: Rect) -> list[str]:
    width = abs(rect.x2 - rect.x1)
    height = abs(rect.y2 - rect.y1)
    
    left = min(rect.x1, rect.x2)
    bottom = min(rect.y1, rect.y2)
    start_pos = Pos(left, bottom, 0.0)
    ans = program_goto_pos(pos, start_pos)
...
def program_clean_rect(pos: Pos, rect: Rect, inset: float) -> list[str]:
    program: list[str] = []
    cur = pos
    left = min(rect.x1, rect.x2)
    bottom = min(rect.y1, rect.y2)
    start_pos = Pos(left, bottom, 0)
